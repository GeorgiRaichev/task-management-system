...
class ProjectGroupsController {
    ...
    private async notifyAddedMembers(
        memberIds: string[],
        senderId: string,
        projectId: string,
        projectName: string
        ...
        await Promise.all(
            uniqueMemberIds.map((recipientId) =>
                createNotification({
                    recipientId,
                    senderId,
                    projectId,
    ...
    public createProjectGroup = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            for (const member of members) {
                ...
                membersMap.set(member.userId, member.role || ProjectGroupMemberRole.MEMBER);
            }
            ...
            await this.notifyAddedMembers(memberIds, req.user.userId, projectId, project.name);
    ...
    public updateProjectGroup = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const updatedGroup = await ProjectGroupModel.findByIdAndUpdate(groupId, req.body, {
                new: true,
                runValidators: true
            })
                .populate('project', 'name status deadline')
