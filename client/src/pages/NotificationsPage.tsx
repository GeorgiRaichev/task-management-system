...
const notificationTitleLabels: Record<string, TranslationKey> = {
    'task assigned': 'taskAssigned',
    'task status changed': 'taskStatusChanged',
    'new comment on task': 'commentAdded',
    'new comment on assigned task': 'commentAdded',
    'attachment added': 'attachmentAdded'
};

const taskStatusLabels: Record<string, TranslationKey> = {
    todo: 'todo',
...
const getNotificationTitleLabel = (title: string, type: string): TranslationKey | null => {
    const normalizedTitle = title.toLowerCase();

    return notificationTitleLabels[normalizedTitle] || getNotificationTypeLabel(type);
};
...
const getTranslatedNotificationMessage = (
    message: string,
    translate: (key: TranslationKey) => string
) => {
    return Object.entries(taskStatusLabels).reduce((currentMessage, [status, label]) => {
        const statusRegex = new RegExp(`\\b${escapeRegExp(status)}\\b`, 'gi');

        return currentMessage.replace(statusRegex, translate(label));
    }, message);
};

const getNotificationColor = (notification: AppNotification): ChipProps['color'] => {
                ...
                {isLoading || isFetching ? (
                    ...
                    <Stack>
                        {notifications.map((notification) => {
                            const titleLabel = getNotificationTitleLabel(
                                notification.title,
                                notification.type
                            );
