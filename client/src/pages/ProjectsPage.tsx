...
import { ProjectStatus, type CreateProjectRequest, type Project } from '../features/projects/types';
import { useTranslate } from '../hooks/useTranslate';
import { getApiErrorMessage } from '../utils/api-error';
import { formatDate } from '../utils/date';
import { canCreateProject, canDeleteProject, canEditProject } from '../utils/permissions';
...
export default function ProjectsPage() {
    ...
    const [projectWithoutGroup, setProjectWithoutGroup] = useState<Project | null>(null);
    ...
    const isPageLoading = isLoading || isFetching || isGroupsLoading || isGroupsFetching;
    const hasAnyProjectActions = projects.some(
        (project) => canEditProject(project, user) || canDeleteProject(project, user)
    );

    const projectIdsWithGroups = useMemo(() => {
        const ids = new Set<string>();

        groupsData?.groups.forEach((group) => {
            if (group.project?._id) {
                ids.add(group.project._id);
            }
        });
    ...
    const handleOpenProjectBoard = (project: Project) => {
        if (!projectIdsWithGroups.has(project._id)) {
            setProjectWithoutGroup(project);
            return;
        }
    ...
    const handleCloseGroupWarning = () => {
        setProjectWithoutGroup(null);
    };
    ...
    return (
        <Stack spacing={3}>
                        ...
                        <Table>
                            <TableHead>
                                ...
                                >
                                    ...
                                    <TableCell align="right">{translate('open')}</TableCell>
                                            ...
                                            <TableCell align="right">
                                                <IconButton
                                                    ...
                                                    onClick={(event) => {
                                                        ...
                                                        handleOpenProjectBoard(project);
                                                    }}
                                                >
            ...
            <Dialog
                open={Boolean(projectWithoutGroup)}
                onClose={handleCloseGroupWarning}
                fullWidth
                maxWidth="xs"
            >
                <DialogTitle sx={{ pb: 1 }}>
                    <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}>
                        ...
                        <Box>
                            <Typography variant="h6" sx={{ fontWeight: 900 }}>
                                {projectWithoutGroup?.name || translate('projects')}
                            </Typography>
                ...
                <DialogActions sx={{ px: 3, pb: 3 }}>
                    <Button variant="contained" onClick={handleCloseGroupWarning}>
                        {translate('cancel')}
                    </Button>
                </DialogActions>
            </Dialog>
